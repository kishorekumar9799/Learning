        ...
        <div className="flex flex-col md:flex-row gap-4 justify-between items-start md:items-center">
          ...
          <div className="relative w-full md:w-80">
            ...
            <input 
              ...
              className="w-full pl-10 pr-4 py-2.5 bg-[#fbfaf7] dark:bg-[#02130e]/80 border border-[#0b3b2c]/10 dark:border-emerald-850 rounded-xl text-sm focus:outline-none focus:border-emerald-500 placeholder:text-slate-400 text-slate-800 dark:text-slate-100 transition-colors"
            />
          </div>
        </div>

        {/* Filters Group */}
        <div className="flex flex-wrap gap-2.5 items-center border-t border-[#0b3b2c]/10 dark:border-emerald-800/15 pt-5">
            ...
            <button
              ...
              className={`px-4 py-1.5 rounded-lg text-xs font-semibold cursor-pointer transition-colors ${
                degreeFilter === deg 
                  ...
                  : 'bg-emerald-55/10 hover:bg-emerald-55/20 dark:bg-emerald-950/20 dark:hover:bg-emerald-950/35 text-slate-600 dark:text-slate-300'
              }`}
            >
            ...
            return (
              <motion.div
                ...
                className={`rounded-2xl border transition-all overflow-hidden cursor-pointer select-none ${
                  isSelected 
                    ? 'border-[#0b3b2c] bg-[#0b3b2c]/5 dark:border-emerald-400 dark:bg-emerald-950/20 shadow-lg' 
                    : 'border-[#0b3b2c]/10 bg-[#fdfcfb] dark:border-emerald-800/15 dark:bg-[#031a14] shadow-xs'
                }`}
              >
                <div className="h-32 relative overflow-hidden bg-slate-100">
                  <div className="absolute inset-0 bg-gradient-to-t from-[#02130e] via-slate-900/40 to-transparent z-10" />
                ...
                <div className="p-5 space-y-4">
                  ...
                  <div className="flex items-center justify-between text-xs font-semibold pt-3 border-t border-[#0b3b2c]/10 dark:border-emerald-800/25">
                    <span className="text-slate-400 font-mono">
                      {programs.filter(p => p.departmentId === dept.id).length} Degree Streams
                    </span>
                    <span className={`flex items-center gap-1 font-mono transition-colors ${isSelected ? 'text-[#0b3b2c] dark:text-emerald-400' : 'text-slate-400'}`}>
          ...
          <div className="grid grid-cols-1 gap-4">
            {filteredPrograms.map((prog) => {
              ...
              return (
                <div 
                  key={prog.id}
                  className="bg-[#fdfcfb] dark:bg-[#031d16]/40 border border-[#0b3b2c]/10 dark:border-emerald-850/30 rounded-xl overflow-hidden transition-all duration-300 shadow-xs hover:shadow-md"
                >
                  <div 
                  ...
                  >
                    <div className="space-y-1.5">
                      <div className="flex flex-wrap items-center gap-2">
                        <span className="px-2.5 py-0.5 rounded bg-[#0b3b2c] text-[#fbfaf7] dark:bg-emerald-500 dark:text-slate-950 font-mono text-[10px] font-bold">
                          {prog.degree}
                        </span>
                    ...
                    <div className="flex items-center gap-6 text-sm text-slate-500 shrink-0">
                      ...
                      <span className="text-xs text-[#b45309] dark:text-emerald-400 font-semibold font-mono">
                  ...
                  {isExpanded && (
                    <motion.div 
                      ...
                      transition={{ duration: 0.3 }}
                      className="border-t border-[#0b3b2c]/10 dark:border-emerald-800/15 bg-emerald-55/10 dark:bg-[#02130e]/40 p-5 md:p-6 space-y-5"
                    >
                        ...
                        <ul className="grid grid-cols-1 md:grid-cols-2 gap-2 text-xs">
                          {prog.requirements.map((req, i) => (
                            <li key={i} className="flex items-center gap-2 pl-3.5 relative text-slate-600 dark:text-slate-300 font-mono">
                              <span className="absolute left-0 w-1.5 h-1.5 rounded-full bg-[#0b3b2c]" />
                              <span>{req}</span>
                            </li>
                          ))}
                        </ul>
